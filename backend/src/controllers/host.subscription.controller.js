// ==========================================
// HOST SUBSCRIPTION CONTROLLER
// ==========================================
// Handles subscription management with proration support
// Last Updated: Removed blogPost references from checkTierLimits
...
} from "../services/tier.change.service.js";
import {
  getArchivedListings,
  restoreListing,
} from "../services/listing.archive.service.js";
...
// CHECK TIER LIMITS
// ==========================================
// Note: This endpoint returns listing-based limits only
// Hosts use ListingStory (1 per listing) instead of BlogPost
// Stories are tied to listings, so story limit = listing limit
...
export const checkTierLimits = async (req, res) => {
  try {
    ...
    const [listingCount, overLimitCount, archivedCount] = await Promise.all([
      // Active listings (includes ACTIVE, INACTIVE, PENDING)
      ...
      // Over-limit listings (need selection during downgrade)
      prisma.listing.count({
        where: {
          hostId,
          status: "OVER_LIMIT",
      ...
      // Archived listings (can be restored with upgrade)
      prisma.listing.count({
        where: {
          hostId,
          status: "ARCHIVED",
...
// ==========================================
// LEGACY CHANGE TIER (Backward compatibility)
// ==========================================
