...
// Last Updated: Removed maxBlogPostsPerMonth references
 ...
 * Access: FINANCE, ANALYST, SUPER_ADMIN
 */
...
export const updateTier = async (req, res) => {
  try {
    ...
    if (
      ...
      (commissionPercent < 0 || commissionPercent > 50)
    ) {
      return res
        .status(400)
        .json({ error: "Commission must be between 0% and 50%" });
    }
  ...
  try {
    ...
    for (const p of pricing) {
      ...
      if (!BILLING_CYCLES[billingCycle]) {
        return res
          .status(400)
          .json({ error: `Invalid billing cycle: ${billingCycle}` });
      }
  ...
  try {
    ...
    await prisma.subscriptionHistory.create({
      data: {
        ...
        action:
          subscription.tier.sortOrder < newTier.sortOrder
            ? "upgraded"
            : "downgraded",
        fromTier: subscription.tier.name,
        toTier: newTier.name,
        details: { reason, extendDays, adminId: req.user.userId },
  ...
  try {
    ...
    await prisma.subscriptionHistory.create({
      data: {
        ...
        details: { reason, immediate, adminId: req.user.userId },
        changedBy: req.user.userId,
        changedByType: "admin",
      },
    });
  ...
  try {
    ...
    await prisma.subscriptionHistory.create({
      data: {
        subscriptionId: subscription.id,
        action: "extended",
        details: { days, reason, adminId: req.user.userId, newEndDate },
        changedBy: req.user.userId,
        changedByType: "admin",
      },
    });
...
/**
 * Seed default tiers and pricing
 * Access: SUPER_ADMIN only
 * Note: Only run once during initial setup
 */
export const seedDefaultTiers = async (req, res) => {
  try {
    ...
    if (existingTiers > 0) {
      return res
        .status(400)
        .json({ error: "Tiers already exist. Use update instead." });
    }
