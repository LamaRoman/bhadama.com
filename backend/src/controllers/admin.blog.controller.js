...
export const approveComment = async (req, res) => {
  try {
    ...
    await prisma.abuseReport.updateMany({
      ...
      data: { resolved: true, adminId: req.user.userId }
    });
  ...
  try {
    ...
    const blog = await prisma.blogPost.create({
      data: {
        ...
        metaDescription: metaDescription || content.substring(0, 170),
        focusKeyword,
        wordCount,
        readingTime,
        allowComments,
        ...
        moderatedBy: status === "PUBLISHED" ? req.user.userId : null,
        moderatedAt: status === "PUBLISHED" ? new Date() : null
      }
    });
...
export const updateBlog = async (req, res) => {
  try {
    ...
    if (updates.title) {
      ...
      let slug = baseSlug;
      let counter = 1;
      while (await prisma.blogPost.findFirst({ where: { slug, id: { not: parseInt(id) } } })) {
        slug = baseSlug + "-" + counter;
        counter++;
      }
    ...
    ["excerpt","category","tags","coverImage","coverImageAlt","metaTitle","metaDescription","focusKeyword","allowComments"].forEach(f => {
    ...
    const blog = await prisma.blogPost.update({ where: { id: parseInt(id) }, data: updateData });
