...
// ==========================================
// Displays current subscription, usage limits, and payment history
// Last Updated: Removed blog posts section
...
export default function SubscriptionTab({ onTabChange }) {
  ...
  const fetchData = async () => {
    ...
    try {
      const [subRes, limitsRes, paymentsRes] = await Promise.all([
        ...
        api("/api/payments/history?limit=5").catch(() => ({
          error: true,
          payments: [],
        })),
      ]);
  ...
  const handleCancelSubscription = async () => {
    if (
      !confirm(
        "Are you sure you want to cancel your subscription? You will retain access until the current period ends."
      )
    ) {
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Current Plan Card */}
      <div
        className={`rounded-2xl border-2 ${tierColors.border} overflow-hidden`}
      >
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-700 mt-1">
                    Your trial ends on {formatDate(sub.trialEndDate)}. Add a
                    payment method to continue after trial.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl border p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-4 bg-gray-50 rounded-xl">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-medium">
                  {limits.limits.listings?.used || 0} /{" "}
                  {limits.limits.listings?.unlimited
                    ? "∞"
                    : limits.limits.listings?.max || 2}
                </span>
              </div>
              ...
              {limits.limits.listings?.archived > 0 && (
                <p className="text-xs text-gray-500 mt-2">
                  {limits.limits.listings.archived} archived listing(s)
                </p>
              )}
            </div>
            ...
            <div className="p-4 bg-gray-50 rounded-xl">
              ...
              <p className="text-2xl font-bold text-gray-900">
                {limits.limits.photosPerListing || 5}
              </p>
            ...
            <div className="p-4 bg-gray-50 rounded-xl">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm text-gray-600">Featured Slots</span>
              </div>
              <p className="text-2xl font-bold text-gray-900">
                {limits.limits.featuredSlots || 0}
              </p>
              <p className="text-xs text-gray-500 mt-1">
                {limits.limits.featuredSlots > 0
                  ? "Available to use"
                  : "Upgrade to get featured slots"}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <p className="text-sm text-purple-200">
                  Get more features & lower commission
                </p>
              </div>
            </div>
        ...
        <button
          onClick={() =>
            onTabChange
              ? onTabChange("support")
              : router.push("/host/dashboard?tab=support")
          }
          className="p-4 bg-white border rounded-xl hover:bg-gray-50 transition-all flex items-center justify-between"
        >
          <div className="flex items-center gap-3">
        ...
        {payments.length > 0 ? (
          ...
          <div className="p-8 text-center text-gray-500">
            ...
            <p className="text-sm mt-1">
              Payments will appear here after you subscribe to a plan
            </p>
          </div>
        )}
      </div>
