...
import MyStoryTab from "./components/tabs/MyStoryTab";
...
const TABS = [
  ...
  { id: "story", label: "My Story", icon: FileText },
...
export default function HostDashboard() {
  ...
  const [counts, setCounts] = useState({ listings: 0, reviews: 0, bookings: 0 });
  const [bannerDismissed, setBannerDismissed] = useState(false);

  // Fetch story status
  const { story, fetchStory, loading: storyLoading } = useHostStory();
  ...
  useEffect(() => {
    if (user && user.role === "HOST") {
      fetchStory().catch(() => {}); // Silent fail - story might not exist
    }
  }, [user, fetchStory]);
  ...
  const handleRefresh = useCallback(() => {
    setRefreshKey(k => k + 1);
    fetchStory().catch(() => {}); // Refresh story status too
  }, [fetchStory]);
  ...
  // Navigate to story tab from banner
  const handleNavigateToStory = useCallback(() => {
    setActiveTab("story");
    setBannerDismissed(false); // Reset dismiss when they navigate to story
  }, []);
  ...
  const shouldShowBanner = !bannerDismissed && 
    !storyLoading && 
    activeTab !== "story" && // Don't show on story tab itself
    (!story || story.status === "DRAFT" || story.status === "REJECTED");
  ...
  const renderTabContent = () => {
    ...
    switch (activeTab) {
      ...
      case "story":
        return <MyStoryTab key={refreshKey} {...props} />;
      case "earnings":
          ...
          <div className="flex border-b border-gray-200 mt-4 overflow-x-auto scrollbar-hide">
            {TABS.map(tab => {
              ...
              const count = 
                ...
                tab.id === "bookings" ? counts.bookings : 0;
              
              // Show indicator on story tab if action needed
              const showStoryIndicator = tab.id === "story" && 
                (!story || story.status === "DRAFT" || story.status === "REJECTED");
