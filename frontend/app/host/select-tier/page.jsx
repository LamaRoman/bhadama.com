...
// TIER SELECTION PAGE
// ==========================================
// Allows hosts to view, compare, and select subscription tiers
// Last Updated: Removed maxBlogPostsPerMonth from features
...
// Note: Removed maxBlogPostsPerMonth - hosts use ListingStory instead
const FEATURE_LIST = [
...
export default function TierSelectionPage() {
  ...
  const currentTierName =
    currentSubscription?.currentTier?.name ||
  ...
  const handleSelectTier = async (tier) => {
    ...
    try {
      if (relation === "upgrade") {
        router.push(
          `/host/upgrade-preview?tierId=${tier.id}&cycle=${selectedBillingCycle}`
        );
        return;
      }
  ...
  const handleNewSubscription = async (tier) => {
    ...
    try {
      ...
      if (data.requiresPayment) {
        router.push(
          `/host/payment?payment=${data.payment?.id || data.paymentId}`
        );
      } else {
              ...
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                    TIER_COLORS[currentTierName]?.iconBg || "bg-gray-200"
                  }`}
                >
                  {(() => {
                    ...
                    return (
                      <Icon
                        className={`w-5 h-5 ${
                          TIER_COLORS[currentTierName]?.text || "text-gray-700"
                        }`}
                      />
                    );
                  })()}
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {currentSubscription.subscription?.billingCycle?.toLowerCase() ||
                      "Free"}{" "}
                    billing
            ...
            return (
              <div
                ...
                className={`relative rounded-2xl border-2 ${colors.border} ${
                  colors.bg
                } p-6 transition-all hover:shadow-xl flex flex-col ${
              ...
              >
                ...
                <ul className="space-y-3 mb-6 flex-1">
                  <li className="flex items-center gap-2 text-sm">
                    ...
                    <span>
                      <strong>{tier.features.maxListings}</strong> listings
                    </span>
                  </li>
                  <li className="flex items-center gap-2 text-sm">
                    <Check className="w-5 h-5 text-green-500 flex-shrink-0" />
                    <span>
                      <strong>{tier.features.commissionPercent}%</strong>{" "}
                      commission
                    </span>
                  </li>
                  <li className="flex items-center gap-2 text-sm">
                    <Check className="w-5 h-5 text-green-500 flex-shrink-0" />
                    <span>
                      <strong>{tier.features.maxPhotosPerListing}</strong>{" "}
                      photos/listing
                    </span>
                  </li>
                  {tier.features.featuredListingSlots > 0 && (
                    <li className="flex items-center gap-2 text-sm">
                      <Check className="w-5 h-5 text-green-500 flex-shrink-0" />
                      <span>
                        <strong>{tier.features.featuredListingSlots}</strong>{" "}
                        featured slots
                      </span>
                    </li>
                  )}
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              ...
              <p className="text-gray-600 text-sm">
                When you upgrade, you only pay the prorated difference for the
                remaining days in your billing period. Your new features are
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              ...
              <p className="text-gray-600 text-sm">
                Your plan changes immediately. If you have more listings than
                your new plan allows, you'll have 7 days to choose which ones to
                keep active. The rest will be archived (not deleted).
              </p>
            </div>

            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h3 className="font-semibold text-gray-900 mb-2">
                Can I restore archived listings?
              </h3>
              <p className="text-gray-600 text-sm">
                Yes! Archived listings are never deleted. You can restore them
                anytime by upgrading to a plan with more listing slots.
              </p>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              ...
              <p className="text-gray-600 text-sm">
                In Nepal, we accept eSewa and Khalti. For international
                payments, we use Dodo Payments which supports major credit/debit
                cards.
              </p>
            </div>
          </div>
